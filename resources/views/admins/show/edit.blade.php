...
@section('title','演出修改 - 演出管理 - Piao.com管理后台')
@section('css')
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form class="form-horizontal m-t" action="{{ URL('/admin/show/'.$res->sid) }}" method="post" enctype="multipart/form-data">
                        @if(count($errors) > 0)
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <input name="sname" class="form-control" type="text" value="{{ $res->sname  }}">
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <input class="laydate-icon form-control layer-date" value="{{ $res->stime  }}" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" name="stime">
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <select class="form-control" name="vid">
                                    <option value="0">--请选择--</option>
                                    @foreach($venues as $v)
                                        <option value="{{ $v->vid }}" {{ $res->vid == $v->vid ? 'selected' : '' }}>{{ '['.$v->vcity.'] '.$v->vname }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                ...
                                <div class="radio i-checks radio-inline">
                                    <label>
                                        <input type="radio" value="3" name="status" {{ $res->status=='3' ? 'checked' : '' }}> <i></i> 特殊
                                    </label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <script id="editor" name="sdesc" type="text/plain">{!! $res->sdesc !!}</script>
                            </div>
                        </div>
                        <div class="form-group m-t-lg">
                            <div class="col-sm-5 col-sm-offset-3">
                                {{ csrf_field() }}
                                {{ method_field('PUT') }}
                                <input type="hidden" name="vid" value="{{ $res->sid }}">
